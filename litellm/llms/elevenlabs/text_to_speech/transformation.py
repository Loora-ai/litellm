...
class ElevenLabsTextToSpeechConfig(BaseTextToSpeechConfig):
    ...
    def map_openai_params(
        ...
        # Handle voice_settings - can come from params, kwargs, or extra_body
        # Merge all sources into a single voice_settings dict
        voice_settings: Dict[str, Any] = {}

        # Check params for voice_settings
        if isinstance(params.get("voice_settings"), dict):
            voice_settings.update(params.pop("voice_settings"))

        # Check kwargs for voice_settings (direct param)
        if isinstance(passthrough_kwargs.get("voice_settings"), dict):
            voice_settings.update(passthrough_kwargs.pop("voice_settings"))

        # Check extra_body for voice_settings
        ...
        # Map OpenAI speed param to voice_settings.speed
        ...
        # Add merged voice_settings back to params if not empty
        if voice_settings:
        ...
        # Extract with_timestamps parameter (ElevenLabs-specific for alignment data)
        # Check params first, then kwargs["extra_body"] (proxy passes extra_body in kwargs)
    ...
    def _add_elevenlabs_specific_params(
        ...
        reserved_kwarg_keys = set(all_litellm_params) | {
            ...
            "user",
            "with_timestamps",
            "voice_settings",  # Handled separately in map_openai_params
        }
    ...
    def get_complete_url(
        ...
        # Check if timestamps are requested - appends /with-timestamps to endpoint
