...
def resolve_env_variables_in_dict(params: Optional[dict]) -> Optional[dict]:
    ...
    for key, value in params.items():
        if isinstance(value, str):
            resolved_value = resolve_env_variable(value)
            verbose_proxy_logger.info(
                "resolve_env_variables_in_dict - key: %s, raw: %s, resolved: %s",
                key,
                value,
                resolved_value[:20] + "..." if resolved_value and len(resolved_value) > 20 else resolved_value
            )
...
class HttpPassThroughEndpointHelpers(BasePassthroughUtils):
    ...
    async def make_multipart_http_request(
        ...
        verbose_proxy_logger.info(
            "make_multipart_http_request - final URL query: %s",
            final_url.query.decode("utf-8")[:100] + "..." if len(final_url.query) > 100 else final_url.query.decode("utf-8")
        )
...
async def pass_through_request(  # noqa: PLR0915
    ...
    try:
        ...
        if default_query_params or merge_query_params:
            ...
            verbose_proxy_logger.info(
                "Pass-through query params - merged: %s, encoded: %s",
                {k: v[:20] + "..." if isinstance(v, str) and len(v) > 20 else v for k, v in merged_params.items()},
                encoded_query[:100] + "..." if len(encoded_query) > 100 else encoded_query
            )
...
async def _register_pass_through_endpoint(
    ...
    verbose_proxy_logger.info(
        "Pass-through endpoint registration - path: %s, raw_target: %s, resolved_target: %s",
        endpoint_data.get("path"),
        raw_target,
        target[:80] + "..." if target and len(target) > 80 else target
    )
