...
async def _build_speechace_logging_context(
    ...
    custom_metadata_header = request.headers.get("x-litellm-metadata")
    if custom_metadata_header:
        try:
            custom_metadata = json.loads(custom_metadata_header)
        except (json.JSONDecodeError, TypeError):
            custom_metadata = None
        if isinstance(custom_metadata, dict):
            kwargs["litellm_params"]["metadata"].update(custom_metadata)
